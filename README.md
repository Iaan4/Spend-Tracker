...
- **Multi-user**: today it's single-tenant (one shared key). I'd add a `users`
  table + `user_id` on expenses, JWT login, and hashed credentials.
- **Migrations** (Alembic) instead of `CREATE TABLE IF NOT EXISTS`, and Postgres
  for anything beyond a single instance (SQLite has a single writer).
- **Edit endpoint** (`PATCH /expenses/{id}`) and soft-delete/undo. Delete is a
  hard delete today, which is simple but irreversible.
- **Currency** column and a fixed category list (or a categories table) instead
  of free text; keyset pagination instead of `OFFSET`.
- **Timezones**: "today" and month boundaries use the server date; I'd store the
  user's timezone.
- Rate limiting, structured logging, CI (pytest + ruff), browser-level UI tests.
- Make the insight threshold configurable and compare against a rolling average
  so one unusual month doesn't dominate.
